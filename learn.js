...
function renderIndex() {
  ...
  if (q) {
    const results = [...S.topics.values()].filter(t => `${JSON.stringify(t)} ${JSON.stringify(S.lessons.get(t.id))}`.toLocaleLowerCase().includes(q));
  ...
  $('#index').innerHTML = S.base.modules.map((m,i) => `<details data-module="${m.id}" ${open.has(m.id)?'open':''}><summary><span class="index-number">${pad(i+1)}</span>${escapeHTML(m.shortTitle)}</summary><div class="topic-links">${m.topics.map(t => `<button data-topic="${t.id}" class="${S.mode==='topic'&&t.id===S.topic?'active':''}" ${S.mode==='topic'&&t.id===S.topic?'aria-current="page"':''}>${escapeHTML(t.shortTitle)}</button>`).join('')}</div></details>`).join('');
}
function writeHash() {
...
function tableHTML(table, className) {
  return `<table class="${className}"><thead><tr>${table.headers.map(h=>`<th scope="col">${escapeHTML(h)}</th>`).join('')}</tr></thead><tbody>${table.rows.map(row=>`<tr>${row.map((v,i)=>i===0?`<th scope="row">${escapeHTML(v)}</th>`:`<td>${escapeHTML(v)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}
function nodeMarkup(n) {
  ...
  const content=n.depth===6?(n.table?`<div class="table-node"><button class="mind-content" data-select="${n.id}"><span class="node-level">${level}</span><span class="node-title">${escapeHTML(n.title)}</span></button>${tableHTML(n.table,'mini-table')}</div>`:`<button class="mind-content" data-select="${n.id}"><span class="node-level">${level}</span><span class="node-body">${escapeHTML(n.body).replace(/\n/g,'<br><br>')}</span></button>`):`<button class="mind-content" data-select="${n.id}"><span class="node-level">${level}</span><span class="node-title">${escapeHTML(n.title)}</span></button>`;
...
function moduleReader(){const m=moduleFor(S.module);return `${readerBar()}<div class="reader-main"><div class="reader-eyebrow">第二层 / 框架</div><h1>${escapeHTML(m.title)}</h1><p class="reader-intro">${escapeHTML(m.question)}</p><div class="reader-module-topics">${m.topics.map(t=>`<button data-topic="${t.id}"><h2>${escapeHTML(t.shortTitle)} ↗</h2><p>${escapeHTML(S.lessons.get(t.id).why)}</p></button>`).join('')}</div></div>`;}
...
function existingFlow(t){if(!t.flow)return '';const f=t.flow;const renderSteps=steps=>`<ol>${steps.map(s=>`<li>${escapeHTML(typeof s==='string'?s:`${s.title}：${s.text}`)}</li>`).join('')}</ol>`;return `<details class="original-flow"><summary>再看一遍流程：${escapeHTML(f.title||t.shortTitle)}</summary>${f.lanes?f.lanes.map(l=>`<h3>${escapeHTML(l.title)}</h3>${renderSteps(l.steps)}`).join(''):renderSteps(f.steps)}<p>${escapeHTML(f.note||'')}</p></details>`;}
function partContent(t,l){
  if(S.part==='plain')return `<section class="learn-section detail-anchor" id="read-meaning"><span class="section-kicker">先用一句人话说明白</span><div class="explainer"><p>${escapeHTML(l.plain)}</p></div></section><section class="learn-section why-box detail-anchor" id="read-why"><h2>它解决的是什么问题？</h2><p>${escapeHTML(l.why)}</p></section>${l.mechanism.map((p,i)=>`<section class="learn-section mechanism detail-anchor" id="read-mechanism-${i}"><h2><small>${pad(i+1)}</small>${escapeHTML(p.title)}</h2><p>${escapeHTML(p.text)}</p></section>`).join('')}${existingFlow(t)}<button class="start-learning" data-part="example">用一个例子理解它<span>→</span></button>`;
  if(S.part==='example')return `<span class="example-label">教学示例 · 假设场景</span><section class="example-context detail-anchor" id="read-context"><h2>${escapeHTML(l.example.title)}</h2><p>${escapeHTML(l.example.context)}</p></section><ol class="story-steps">${l.example.steps.map((p,i)=>`<li class="detail-anchor" id="read-step-${i}"><h2><small>${pad(i+1)}</small>${escapeHTML(p.title)}</h2><p>${escapeHTML(p.text)}</p></li>`).join('')}</ol><section class="takeaway detail-anchor" id="read-takeaway"><strong>从这个例子里带走什么</strong><p>${escapeHTML(l.example.takeaway)}</p></section><button class="start-learning" data-part="compare">再比较容易混淆的概念<span>→</span></button>`;
  if(S.part==='compare')return `<section class="learn-section detail-anchor" id="read-table"><h2>放在一起，看看差别在哪里</h2><div class="comparison-scroll" tabindex="0" role="region" aria-label="${escapeHTML(t.shortTitle)}对比表">${tableHTML(l.compare,'comparison-table')}</div></section><section class="pitfall detail-anchor" id="read-pitfall"><h2>常见的误解</h2><p>${escapeHTML(l.pitfall.mistake)}</p><p class="correct"><strong>更准确的理解</strong>${escapeHTML(l.pitfall.correction)}</p></section><button class="start-learning" data-part="check">换一个场景，检查理解<span>→</span></button>`;
  return `<section class="question-box detail-anchor" id="read-question"><h2>先试着用自己的话判断</h2><p>${escapeHTML(l.check.question)}</p></section><details class="answer"><summary>展开参考思路</summary><p>${escapeHTML(l.check.answer)}</p></details><p class="level-note">重点是能说明理由和边界，不需要背诵固定答案。</p>`;
}
...
function bindUI(){
  document.addEventListener('click',e=>{
    const target=e.target.closest('[data-topic],button[data-module],[data-part],[data-overview],[data-reader-close],[data-original]');if(!target)return;
    if(target.dataset.topic)showTopic(target.dataset.topic);
...
async function init(){
  try{
    ...
    const data=await Promise.all(files.map(async file=>{const r=await fetch(`./${file}`);if(!r.ok)throw new Error(`无法读取 ${file}`);return r.json();}));
    S.base=data[0];data.slice(1).flatMap(d=>d.topics).forEach(t=>S.lessons.set(t.id,t));
    buildKnowledgeTree();bindUI();S.ready=true;$('#loading').hidden=true;loadRoute();
