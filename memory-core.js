...
(function(root){
  ...
  const isStudyTerm=t=>t.learning?.status!=='reference';
  ...
  function normalize(raw,terms){
    const state=defaults(),ids=new Set(terms.map(t=>t.id)),activeIds=new Set(terms.filter(isStudyTerm).map(t=>t.id));if(!raw||raw.version!==1)return state;
    ...
    const s=raw.session;
    if(s&&Array.isArray(s.ids)&&s.ids.every(id=>activeIds.has(id))&&new Set(s.ids).size===s.ids.length&&Number.isInteger(s.position)&&s.position>=0&&s.position<=s.ids.length){
  ...
  function candidates(terms,state){const f=state.settings,q=f.query.trim().toLowerCase();return terms.filter(t=>{
    ...
    return isStudyTerm(t)&&(f.module==='all'||t.moduleIds.includes(f.module))&&(f.status==='all'||f.status===status||(f.status==='weak'&&['unknown','unsure'].includes(status)))&&(!q||[t.zh,t.en,t.abbr,...(t.aliases||[]),t.learning?.focus,...(t.learning?.checkpoints||[])].join(' ').toLowerCase().includes(q));
  });}
  ...
  function start(state,terms,rng=Math.random,onlyId){
    ...
    let list=shuffled(onlyId?terms.filter(t=>t.id===onlyId&&isStudyTerm(t)):candidates(terms,state),rng);
  ...
  function counts(state,terms){const out={new:0,unknown:0,unsure:0,known:0};for(const t of terms.filter(isStudyTerm))out[state.records[t.id]?.status||'new']++;return out;}
  const API={defaults,normalize,candidates,start,reveal,rate,undo,counts,isStudyTerm,STATUSES};
