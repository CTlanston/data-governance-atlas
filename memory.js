...
window.AtlasMemory = (() => {
  ...
  function learningLinks(t){
    const links=t.learning?.links||[];
    if(!links.length)return '';
    return `<section class="card-contexts"><h4>按场景理解 · 直达考点</h4><div>${links.map(l=>`<a href="#${esc([l.topicId,l.sectionId,l.itemId].join('/'))}" class="card-knowledge-link"><span>${esc(l.relation)}</span><strong>${esc(l.label)} ↗</strong></a>`).join('')}</div></section>`;
  }
  function checkpoints(t){return t.learning?.checkpoints?.length?`<section class="card-checkpoints"><h4>掌握到什么程度</h4><ul>${t.learning.checkpoints.map(c=>`<li>${esc(c)}</li>`).join('')}</ul></section>`:'';}
  function card(){
    ...
    if(!t)return done();
    const dir=s.directions[s.position],enFirst=dir==='en-zh',status=state.records[t.id]?.status||'new',modules=t.moduleIds.map(id=>base.modules.find(m=>m.id===id)?.shortTitle).filter(Boolean);
    return `<div class="round-progress"><span>本轮 <strong>${s.position+1}</strong> / ${s.ids.length}</span><span>已自评 ${s.position} 张</span></div><div class="round-track" role="progressbar" aria-label="本轮测试进度" aria-valuenow="${s.position}" aria-valuemin="0" aria-valuemax="${s.ids.length}"><i style="width:${s.position/s.ids.length*100}%"></i></div><article class="recall-card ${s.revealed?'revealed':''}" tabindex="-1" aria-label="${s.revealed?'已翻开的答案':'待回答的词汇卡'}"><div class="card-meta"><span>${esc(modules.join(' / '))} · ${t.learning?.status==='supporting'?'支撑知识':'核心考点'}</span><span class="recall-status ${status}">${labels[status]}</span></div><div class="recall-question"><div class="recall-direction">${enFirst?'ENGLISH → 中文':'中文 → ENGLISH'}</div><h2>${esc(enFirst?t.en:t.zh)}</h2>${enFirst&&t.abbr?`<div class="recall-abbr">${esc(t.abbr)}</div>`:''}<p>${enFirst?'先回想中文含义，再试着回答下面的考点。':'先说出对应英文，再试着回答下面的考点。'}</p>${t.learning?.focus?`<div class="card-focus"><small>这一词考什么</small><p>${esc(t.learning.focus)}</p></div>`:''}</div>${s.revealed?`<div class="memory-answer" tabindex="-1"><div class="answer-heading"><span>参考答案</span><h3>${esc(enFirst?t.zh:t.en)}${!enFirst&&t.abbr?` <small>${esc(t.abbr)}</small>`:''}</h3></div><section><h4>怎么理解</h4><p>${esc(t.definition)}</p></section><section class="answer-distinction"><h4>别和它混淆</h4><p>${esc(t.distinction)}</p></section><section><h4>举个例子 · 教学示例</h4><p>${esc(t.example)}</p></section>${checkpoints(t)}${learningLinks(t)}<details class="card-sources"><summary>术语来源</summary>${refs(t)}</details></div>`:`<div class="reveal-control"><button class="memory-primary" data-memory="reveal">翻开答案 <kbd>Space</kbd></button><small>先回想，再翻卡；不需要先选“了解”。</small></div>`}</article><div class="memory-rating"><p>${s.revealed?'按自己的理解标记，选择后进入下一张。':'翻开答案后，可以标记自己的理解程度。'}</p><div>${[['unknown','不了解','1'],['unsure','模糊','2'],['known','非常了解','3']].map(([id,label,key])=>`<button class="rate-${id}" data-memory="rate" data-status="${id}" ${s.revealed?'':'disabled'}>${label}<kbd>${key}</kbd></button>`).join('')}</div></div><div class="round-tools"><button data-memory="undo" ${s.history.length?'':'disabled'}>← 撤回上次标记</button><span>自评帮助安排复习，不代表客观测评结果。</span></div>`;
  }
  ...
  function render({preserveQuery=false}={}){
    ...
    host.innerHTML=`<div class="memory-inner"><div class="memory-heading"><div><div class="memory-kicker">TERMINOLOGY FLASHCARDS</div><h1>把专业词汇，变成熟悉的语言。</h1><p>${terms.filter(Core.isStudyTerm).length} 张精选词卡 · 从含义到考点，再到不同场景的应用。</p></div><span class="memory-self">SELF CHECK</span></div>${statistics()}${filters()}<div class="review-area">${card()}</div><p class="memory-storage" role="status"></p></div>`;
